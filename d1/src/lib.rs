...
pub fn do_first_part(file_path: &str) -> Result<u32, String> {
    parse_cals(file_path).and_then(|cals| {
        cals.into_iter()
            .max()
...
pub fn do_sec_part(file_path: &str) -> Result<u32, String> {
    parse_cals(file_path).and_then(|mut cals| {
...
mod d1_tests {
    ...
    #[test]
    fn test_first() {
        let res = do_first_part("./src/test.txt");
    ...
    #[test]
    fn test_sec() {
        let res = do_sec_part("./src/test.txt");
